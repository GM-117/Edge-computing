...
class DataGenerator(object):
    ...
    def solve_instance(self, sequence, tw_open, tw_close):
        ...
        tour, tour_length, delivery_time = self.solver.run(precision * dist_matrix, demands, precision * (1 + tw_open),
                                                           precision * (
                                                                       1 + tw_close))  # a tour is a permutation + start_index    # Rq: +1 for depot offset
    ...
    def gen_instance(self, test_mode=True, seed=0):
        ...
        # Randomly generate (max_length+1) city integer coordinates in [0,100]
        sequence = np.random.randint(100, size=(self.max_length, self.dimension))
        ...
        if self.pretrain == True:
            ...
            tw_open = tw_open[
                      ::-1]  # TW open constraint sorted (reverse order)   Rq: depot = tw_open[-1], tw_width[-1] and sequence[-1]
        # TW constraint 2 (time width): Gaussian or uniform distribution
        tw_width = np.abs(np.random.normal(loc=self.width[0], scale=self.width[1],
                                           size=(self.max_length, 1)))  # gaussian distribution
    ...
    def load_Dumas(self, dir_='n20w100'):
        ...
        for file_name in os.listdir('benchmark/' + dir_):
            ...
            # Coordinates    
            seq = np.stack((x, y),
                           axis=1)  # city integer coordinates in [0,100[ ############################### RESCALE ??
            ...
            # Save data
            dataset[file_name] = {'input_': input_, 'sequence': sequence, 'tw_open': tw_open, 'tw_close': tw_close,
                                  'optimal_sequence': seq[opt_permutation],
                                  'optimal_tw_open': np.expand_dims(t_open[opt_permutation], axis=1),
                                  'optimal_tw_close': np.expand_dims(t_close[opt_permutation], axis=1),
